...
function simulate(
  ...
  while (
    simulatedBricks.some((brick) => brick.status === "visible") &&
...
export async function generateSVG(
  ...
  const colors = await fetchGithubContributionsGraphQL(username, githubToken);
  ...
  const keyTimes = Array.from({ length: states.length }, (_, i) =>
    (i / (states.length - 1)).toFixed(4),
  ).join(";");
